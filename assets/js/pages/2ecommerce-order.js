...
editlist = !1,
options = {
    valueNames: ["id", "product_name", "date", "amount", "payment", "from"],
    page: perPage,
    pagination: !0,
...
}, xhttp.open("GET", "assets/json/2orders-list.init.json"), xhttp.send();
...
orderList.filter(function(e) {
    e = (matchData = (new DOMParser).parseFromString(e.values().payment, "text/html")).body.firstElementChild.innerHTML;
...
// e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Order", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("modal-id").style.display = "none", document.getElementById("exampleModalLabel").innerHTML = "Add Order", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Order") : (document.getElementById("exampleModalLabel").innerHTML = "List Order", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
...
example = (Array.prototype.slice.call(forms).forEach(function(a) {
    a.addEventListener("submit", function(e) {
        var t;
        a.checkValidity() ? (e.preventDefault(), "" === customerNameField.value || "" === productNameField.value || "" === dateField.value || "" === amountField.value || "" === paymentField.value || editlist ? "" !== customerNameField.value && "" !== productNameField.value && "" !== dateField.value && "" !== amountField.value && "" !== paymentField.value && editlist && (t = orderList.get({
            id: idField.value
...
Array.from(document.getElementsByName(" ")).forEach(function(a) {
    a.addEventListener(" ", function(e) {
        ...
        var t = document.querySelectorAll('[name=" "]:checked').length;
